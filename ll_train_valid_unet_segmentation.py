            ...
            if opt.setting == 'FL':
                ...
                if opt.ours_dir + opt.ours_loss + opt.ours_dis + opt.ours_pccs == 2:
                    if opt.ours_dir == 1 and opt.ours_loss == 1:
                        ...
                        print("weight_combinations",weight_combinations)

                        # weight_combinations = [[(a + b) / 2 for a, b in zip(weights[0], weights[1])] for weights in weight_combinations]
                        # weight = weight_combinations
                        # 平均权重计算
                        non_loss_weight_combinations = weight_Dir
                        loss_weight_combinations = weight_pccs

                        # 计算加权平均
                        weight_combinations = (non_loss_weight_combinations + loss_weight_combinations)/2
