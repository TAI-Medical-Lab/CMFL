...
class Unet(nn.Module):
    ...
    def forward(self, ct, pet):
        if opt.setting !='central':
            if dist.get_rank() == 0 or 1:
                x = torch.cat([ct, pet], dim=1)
            elif dist.get_rank() == 2:
                x = torch.cat([pet, pet], dim=1)
            elif dist.get_rank() == 3:
                x = torch.cat([ct, ct], dim=1)
            # if opt.dataset_named == 'ct':
            #     x = ct
            # elif opt.dataset_named == 'pet':
            #     x = pet
            # elif opt.dataset_named == 'petct':
            #     x = torch.cat([ct, pet], dim=1)
        elif opt.setting=='central':
            if opt.dataset_named == 'ct':
                x = ct
            elif opt.dataset_named == 'pet':
                x = pet
