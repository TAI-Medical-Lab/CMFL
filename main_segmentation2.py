...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    if opt.setting != 'central':
        ...
        np.savetxt(f'whole_body_result/{opt.setting}_{opt.dataset_named}_{strroot}_{opt.dataset_choice}_{opt.model_choice}_DP_{opt.DP}_{opt.dp_delta}.txt', result,delimiter=',', fmt='%s')
    else:
        ...
        if dice >= dice_best:
            ...
            torch.save({"model_state_dict": model.state_dict()},
                    "{}s/Unet_augment/model_lr10-4_epoch200_best_{}_{}_{}.pth".format(opt.out, epoch,strroot,opt.model_choice))
        else:
            torch.save({"model_state_dict": model.state_dict()},
                    "{}s/Unet_augment/model_lr10-4_epoch200_{}_{}_{}.pth".format(opt.out,epoch,strroot,opt.model_choice))
