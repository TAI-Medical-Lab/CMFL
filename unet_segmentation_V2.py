...
class Unet(nn.Module):
    def __init__(self,in_ch,out_ch):
        ...
        self.CAM = ChannelAttention(512, ratio=16)
    ...
    def forward(self, ct, pet):
        ...
        c1_B = self.conv1(pet)
        p1_B = self.pool1(c1_B)
        c2_B = self.conv2(p1_B)
        p2_B = self.pool2(c2_B)
        c3_B = self.conv3(p2_B)
        p3_B = self.pool3(c3_B)
        c4_B = self.conv4(p3_B)
        p4_B = self.pool4(c4_B)

        p5 = torch.cat([p4, p4_B], dim=1)

        #加入cbam
        p5_ca = self.CAM(p5) * p5
